...
class BidTrackerScraper:
    ...
    def _read_config(self):
        ...
        self.auction_pages = dict(config['auction-pages'])        
    ...
    def _login(self):
     ...
     #   }     
        ...
        r = self.session.get("http://www.bidtracker.info/Account/Login" )
        print (r.status_code)
        ...
        payload['ctl00$MainContent$UserName'] = self.username
        payload['ctl00$MainContent$Password'] = self.password
        payload['ctl00$MainContent$ctl05'] = "Log+in"

        time.sleep(4)
        post = self.session.post("http://www.bidtracker.info/Account/Login", data=payload)
        print (post.status_code)

        time.sleep(4)

    def _get_all_auction_ids_for_group(self, auction_group):
        url = self.auction_pages[auction_group]
    ...
    def _scrape_auction_group(self, auction_group):
        ...
        for aID in auction_ids:
            ...
            if not (self.pages_collection.find_one({"_id": aID}, {"AuctionGroup":auction_group})):
                ...
                time.sleep(3)
                r = self.session.get("http://www.bidtracker.info/Auction?site=quibids&auction_id=" + aID)
                page_dict ["Auction"] =  r.text
                if 'class="sold"' not in r.text:
                    continue
                time.sleep(1)
                r = self.session.get("http://www.bidtracker.info/AuctionTable?site=quibids&auction_id=" + aID)
                page_dict ["AuctionTable"] = r.text
                time.sleep(3)
                r = self.session.get("http://www.bidtracker.info/History?site=quibids&auction_id=" + aID)        
    ...
    def scrape(self):
        ...
        with requests.Session() as self.session:
            self._login()
            for auction_group in self.auction_pages.keys():
